...
def test_the_example_project_builds_and_its_installer_asks_first():
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        with PgfsImage(disc) as img:
            ...
            for name in ("sh", "ls", "cat", "echo", "mkdir", "rmdir", "rm", "mv", "cp", "clear", "more", "edit",
                         "graph", "cube", "files", "corrupter", "splash", "reboot", "img"):
                path = f"/bin/{name}.bin"
    ...
    assert rows[6] == "28 files to copy" and rows[7] == "It will boot /boot.bin", rows
