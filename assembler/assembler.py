...
class Assembler:
    ...
    def _parse_operands(self, mnemonic: str, operand_str: str) -> Tuple[int, int, int, int]:
        ...
        ops = INSTRUCTIONS_BY_NAME[mnemonic].operands
        dst = src1 = src2 = NONE_REG
        imm = 0
        ops = sorted(ops)  # Sort operands by priority (dst, src1, src2, imm)
        print()
        print(mnemonic, operand_tokens)
        print("ops", ops)
        if not ops:
        ...
        for op in ops:
            ...
            if 0 == op: #dst
            ...
            elif 1 == op and 2 in ops: #src1 !src1+imm in ops
                src2 = operand_tokens[1]
            
            elif 1 == op and not 2 in ops: #src1 src1+imm in ops
                print("case 3")
                dst = self._parse_operand(operand_tokens[0])
                ...
                if len(operand_tokens) < 3:
                    # duplicate the destination to src1
                    src1 = dst
                    pigeon = 1
                else:
                    src1 = self._parse_operand(operand_tokens[1])

                src2, imm = self._parse_operand_or_imm(operand_tokens[2-pigeon])
